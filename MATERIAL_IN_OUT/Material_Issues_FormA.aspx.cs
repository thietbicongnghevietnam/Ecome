...
namespace MATERIAL_IN_OUT
{
    public partial class Material_Issues : System.Web.UI.Page
    {
       ...
       // public DataTable dt_IssueMaterial;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (REQUESTID != "" && Dept != null && User_next != "" && Stock != "" && (RoleDept != "" || RoleRQ_Apr != "" || RoleStore != "" || RoleStore_Apr != ""))
                {
                    ...
                    {
                        //out dept
                        LoadData_1RQ_FromEmail(Session["RequestID_1RQ"].ToString(), Session["Role_Dept"].ToString(), Session["Role_Aproved_Dept"].ToString(), Session["RoleOutStock"].ToString(), Session["Role_Aproved_Dept"].ToString().Trim());  //Session["Role_Aproved_Dept"].ToString().Trim()
                    }
                    else 
                    {
            ...
            if (FileUpload1.HasFile)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        string TypeRQ = null; string Material = null; string Sloc = null;  string Mvtype = null; string Plant = null; decimal UnitPriceST = 0;
                        string CostCenter = null; string VendorCode = null; string Note = null; string DateVoucher = null; decimal Amount = 0; string MVName = null; string RQ_Reset = null;
                        ...
                        else
                        {
                            ...
                            else // Đúng định dạng của RQ
                            {
                                ...
                                //DataTable dtCheckMV = DataConn.DataTable_Sql("SELECT COUNT(ISNULL(MVTypeCode,0)) AS CheckMV FROM tbl_MV_Master WHERE MVTypeCode = '" + Mvtype + "'  AND TypeID LIKE '%" + TypeRQ + "%'");                                
                                ...
                                if (TypeRQ != "" && RQ_Reset != "") // Upload RQ
                                {
                                    if (int.Parse(dtCheckRQ.Rows[0]["CheckRQ"].ToString()) > 0)
                                    {
                                        ...
                                        sql_Reset = sql_Reset + "[CountryofOrigin] = '" + CountryOfOrgin + "', [ItemDescription]  = '" + ItemDescription + "',[TypeSapPMS]='"+Type_SAP_PMS+ "',[TypeOutSap]='" + TypeOutSap + "',Flag_price_sap  = '" + 0 + "'";
                                ...
                                else
                                {
                                    ...
                                    {
                                        sql_ = sql_ + " INSERT INTO tbl_RQ_MaterialIssue ([RequestNo],[Material],[TypeID], [IssueQty],[UnitPrice_ST],[MvType],MvName,[Plant],Note,DateVoucher,Amount_ST , VendorCode,CostCenter,Sloc,UserCreate,RQDeptID,CountryofOrigin,ItemDescription,Type_Rosh_Halb,Reason,NameCode,TypeSapPMS,TypeOutSap) ";
                                        sql_ = sql_ + " VALUES( '" + Request_NO + "','" + Material + "','" + TypeRQ + "'," + Qty + "," + UnitPriceST + " ,'" + Mvtype + "','" + MVName + "','" + Plant + "' ,N'" + Note + "', '" + DateVoucher + "'," + Amount + ",'" + VendorCode + "','" + CostCenter + "','" + Sloc + "','" + Session["UserName"].ToString() + "','" + Public_Dept + "','" + CountryOfOrgin + "','" + ItemDescription + "','" + Type_Rosh_Halb + "','" + Reason + "','" + Namecode + "','" + Type_SAP_PMS + "','"+ TypeOutSap + "') ";
            ...
            try
            {
                ...
                if (dtNextMail.Rows.Count > 0)
                {
                    for (int j = 0; j < dtNextMail.Rows.Count; j++)
                    {
                        ...
                        {
                            WriteLog(RQ, $"Loi vong 2_A gui mail row {j}: {exMail.Message}");
                ...
                else 
                {
                    ...
                    WriteLog(RQ, "dtNextMail_A tra ve 0 dong → không có ai nhận mail");
                }
            }
            catch (Exception ex)
            {
                WriteLog(RQ, $"Loi vong 1A SendEmail_Next: {ex.ToString()}");
        ...
        //            dt_ReportAll = DataConn.FillStore("Select_Issue_Material_Report_A2", CommandType.StoredProcedure, Request_NO);
        //            for (int i = 0; i < dt_ReportAll.Rows.Count; i++)
        //            {
        //                string material = dt_ReportAll.Rows[i]["Material"].ToString();
        //                string plant = dt_ReportAll.Rows[i]["Plant"].ToString();
        ...
        //                float Amount_ST = qty_issue * UnitPrice_ST;
        //                dt_update = DataConn.FillStore("Update_Issue_Material_Report_A2", CommandType.StoredProcedure, Request_NO, material, plant, qty_issue, UnitPrice_ST, Amount_ST, UserID);
        //            }
        ...
        protected void btnDelete_requestno(object sender, EventArgs e) 
        {
            ...
            else
            {
                ...
                DataTable dt_update = new DataTable();
                dt_update = DataConn.StoreFillDS("delete_requestno_level1A", CommandType.StoredProcedure, requestname, rolestock, roledept, userid);
        ...
        static void ProcessExcelFile3(string filePath, string newFilePath, string Request_NO, string SanctionName)
        {
            ...
            {
                dtexcel = DataConn.FillStore("Export_ScrapList_Report_A", CommandType.StoredProcedure, Request_NO, SanctionName);
            }
            ...
            using (var package = new ExcelPackage(fileInfo))
            {
                ...
                foreach (DataRow dataRow in dtexcel.Rows)
                {
                    ...
                    worksheet.Cells[row, 5].Value = dataRow["NameCode"];   
                    ...
                    worksheet.Cells[row, 10].Value = "";// dataRow["UnitPrice_AC"]; //form A khong co
                    worksheet.Cells[row, 11].Value = "";// dataRow["Amount_AC"];    //form A khong co


                    worksheet.Cells[row, 12].Value = "";  //cot remark
